...
files = os.path.join(UPLOADFOLDER, 'flaskapprefatorado','app','admin','files','adminuploads')
adm = Blueprint('adm', __name__)
excel.init_excel(current_app)

from ..models.hausz_mapa import (Usuarios, GrupoUsuario, ProdutosSaldos
, ProdutoPrazoProducFornec, DeparaProdutos, ColetadosDiario, 
ProdutoDetalhe,ProdutoBasico, ProdutoMarca,ArquivosConvertidos)
...
class MyAdminIndexView(AdminIndexView):
    ...
    def Home(self):
        ...
        return self.render('admin/index.html', produtosc=produtos_crossdocking, cont_natualizado=cont_natualizado
                           , produtoe=produtos_disponivel, cont_estoque=cont_estoque, atualizado_dia_marca = total_marcas, tot_nao_feito=naofeito)
    # pass
    # def is_accessible(self):
    #   return current_user.is_authenticated
...
class UploadfilesView(BaseView):
    ...
    def upload_arquivos(self):
        files = os.listdir(os.path.join(UPLOADFOLDER,'flaskapprefatorado','app','uploads'))
        print('caminhoarquivoooo',files)
    ...
    def upload_files(self):
        ...
        if filename != '':
            ...
            uploaded_file.save(os.path.join(UPLOADFOLDER,'flaskapprefatorado','app','uploads',filename))
            dicts = Produto(os.path.join(UPLOADFOLDER,'flaskapprefatorado','app','uploads',filename))
    ...
    def export_modelo_alteracao_saldo(self):
        return excel.make_response_from_array([['SKU','MARCA','SALDO','PRAZO']]
...
class NotificationsView(BaseView):
    ...
    def marcas_atualizadas_dia(self):
        ...
        return self.render('admin/semcadastro.html', produtos = lista_dicts)
    ...
    def nao_atualizados(self):
        ...
        return self.render('admin/produtoatualizado.html', produtos = lista_dicts, )
    ...
    def referencia_marcas(self):
        ...
        return self.render('admin/referenciamarcas.html', produtos=lista_produtos)
...
class ArquivosConvertidosView(ModelView):
    ...
    column_searchable_list = ['SKU']
    can_view_details = True
    column_list = ['SKU', 'idarquivo','saldoproduto','dataatualizado','marca']
...
class ProdutoPrazoProducFornecView(ModelView):
    ...
    page_size = 15
    create_modal = True
    column_sortable_list = ('IdPrazos', ('IdPrazos', ProdutoPrazoProducFornec.IdPrazos))
    column_display_pk = True
    column_searchable_list = ['SKU']
    column_list = ['SKU','PrazoEstoqueFabrica','PrazoProducao','PrazoOperacional','PrazoFaturamento','PrazoTotal']
    column_filters = ['SKU']
    can_create = True
    can_edit = True
    Can_delete = True
...
class ArquivosConvertidosView(ModelView):
    create_modal = True
    column_display_pk = True
    column_searchable_list = ['idarquivo', 'SKU', 'saldoproduto', 'IdMarca'
        , 'marca']

    an_view_details = True
    column_list = ['idarquivo', 'SKU', 'saldoproduto', 'IdMarca'
        , 'marca']
    # column_default_sort = ('last_name', False)
    column_filters = [
...
class IndesView(BaseView):
    ...
    def Home(self):
        ...
        return self.render('admin/index.html', produtosc=produtos_crossdocking, cont_natualizado=cont_natualizado
                          , produtoe=produtos_disponivel, cont_estoque=cont_estoque, nao_tot = cont_dia_nao_atualizado)


path = os.path.join(UPLOADFOLDER,'flaskapprefatorado','app','admin','files','adminuploads')
...
admin.add_view(ProdutoPrazoProducFornecView(ProdutoPrazoProducFornec, db.session, category="Produtos"))
...
admin.add_link(MenuLink(name='ReferenciaMarcas', url='/referenciamarca',category="AtualizacaoProdutos"))
admin.add_view(FileAdmin(path, '/adminuploads/', name='/adminuploads',category="AtualizacaoProdutos"))
admin.add_link(MenuLink(name='UploadArquivosFornecedor', url='/uploadestoques',category="AtualizacaoProdutos"))
admin.add_link(MenuLink(name='UploadArquivos', url='/uploadestoques',category="AtualizacaoProdutos"))
admin.add_sub_category(name="AtualizacaoProdutos", parent_name="AtualizacaoProdutos")
admin.add_link(MenuLink(name="MonitoramentoDePrecos", url="/monitoramentodeprecos/1"))
